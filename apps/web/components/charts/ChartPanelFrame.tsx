...
export function ChartPanelFrame({
  ...
  return (
    <section className={`rounded-[var(--radius)] border border-[var(--border)] bg-[var(--bg-surface)] p-5 shadow-sm ${className}`}>
      {title ? (
      ...
      {lastUpdatedLabel ? (
        <p className="mt-2 text-xs text-[var(--text-muted)]">{lastUpdatedLabel}</p>
      ) : null}
