            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 border border-gray-100 rounded-lg p-4">
                ...
                <div className="text-3xl font-black mt-1 tabular-nums text-[var(--text-primary)]">
                  ${Number.isFinite(data.estimated_value) ? (data.estimated_value).toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 }) : "N/A"}
                </div>
              </div>
              <div className="bg-gray-50 border border-gray-100 rounded-lg p-4">
                <span className="text-xs text-[var(--text-muted)] font-semibold uppercase tracking-wider">Market Dislocation</span>
                <div className={`text-3xl font-black mt-1 tabular-nums ${data.upside_pct >= 0 ? "text-[var(--surface)]" : "text-[var(--delta-down)]"}`}>
                  {data.upside_pct > 0 ? "+" : ""}{Number.isFinite(data.upside_pct) ? data.upside_pct.toFixed(1) : "0.0"}%
                </div>
              </div>
            </div>
