...
- [ ] Preserve legacy and stable metric variants side-by-side instead of switching fields in place.
  - Example target shape:
    - `roic_legacy`
    - `roic_stable_v2`
    - `growth_avg_legacy`
    - `growth_cagr_v2`
- [ ] Ensure each decision-grade metric carries explicit metadata from `guideline/suggestion.md`:
  - `method`
  - `quality`
  - `confidence`
  - calculation notes or warnings where fallback logic applies
- [ ] Review whether all frontend-consumed responses expose stable version identifiers consistently enough to support auditability and future formula changes.
- [ ] Confirm main UI views display only CAGR as Growth while keeping average YoY growth confined to audit or detail contexts.
- [ ] Confirm the corporate audit response remains a single unified payload containing at least:
  - `roic`
  - `growth`
  - `dcf`
  - `wacc`
...
- [ ] Add pure-finance tests for the new ROIC and growth helpers under `tests/core_finance/`.
...
- [ ] Update route or service tests under `tests/api/` to verify:
...
- [ ] Update any fixtures or E2E mocks that currently assume the old denominator or growth-basis behavior.
