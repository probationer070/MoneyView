...
def render(df):
    ...
    # 1. 국채 10년물 금리 추이
    st.markdown("##### 1. 국고채 10년물 금리 급등 구간")
    gov_bond = df[df['name'].str.contains("국고채") & df['name'].str.contains("10년")].copy()
    
    if not gov_bond.empty:
        fig1 = px.line(gov_bond, x='date', y='value', markers=True, title="국고채 10년물 금리", labels={'value': '금리(%)'})
    ...
    with col2:
        ...
        if not gov_bond.empty and not loan_rate.empty:
            merged = pd.merge(gov_bond[['date', 'value']], loan_rate[['date', 'value']], on='date', suffixes=('_bond', '_loan'))
