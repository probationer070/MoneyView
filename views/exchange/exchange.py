...
def render(df):
    ...
    with col2:
        ...
        kr_cpi = df[df['name'] == "CPI한국"].copy()
        us_cpi = df[df['name'] == "CPI미국"].copy()
        ...
        if not krw_usd.empty and not kr_cpi.empty and not us_cpi.empty:
            ...
            # 환율 월평균 (일별 -> 월별) -> 이 부분은 app.py에서 단위 처리를 하므로 생략 가능하나 안정성을 위해 유지할 수 있음
            krw_usd = krw_usd.set_index('date').resample('MS')['value'].mean().reset_index()

            # 데이터 병합 (how='outer' 로 최신 데이터를 유실하지 않도록 ffill 처리)
            merged = pd.merge(krw_usd, kr_cpi[['date', 'value']], on='date', how='outer', suffixes=('_rate', '_kr'))
            merged = pd.merge(merged, us_cpi[['date', 'value']], on='date', how='outer') # value는 us_cpi
            ...
            if not merged.empty:
                ...
                fig_reer = px.line(merged, x='date', y='REER', title="실질실효환율 (REER) Proxy (2000=100) - ECOS 데이터",
                                   labels={'REER': '지수', 'date': '날짜'},
                                   hover_data={'date': '|%Y-%m-%d', 'REER': ':.2f'})
