...
from nasdaq import single_snapshot
...
def calc_num():
    with open("./stock_targets.json", "r", encoding="utf-8") as f:
        data = json.load(f)
        print("데이터 :",data.get("targets", []))

    
    return print("데이터 개수:",len(data.get("targets", [])))
