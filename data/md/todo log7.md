...
- [x] Tighten the product boundary recommended in `guideline/suggestion.md`:
  Watchlist = tracking/lightweight comparison
  Portfolio = weighted testing + cash + snapshots + history
- [x] Universe selector UI (dropdown/toggle) for Portfolio and Watchlist views.
- [x] Implement default benchmarks for Korean users (KOSPI + KOSDAQ + 3–4 sector ETFs).

## Next

- [x] Database: Add `comparison_universe` field to daily snapshots for historical reproducibility.
- [x] E2E: Create per-page mock files for clean isolation.
- [x] E2E: Replace the remaining generic/mock-command pattern across all major page tests.
- [ ] Performance: serve latest snapshot via Redis or indexed DB view (target <1s load).

## Risks To Watch

- [x] `Import JSON` still overwrites the DB watchlist intentionally; this remains explicit and armed, not the safe path.
- [x] Watchlist vs Portfolio separation is now enforced more clearly in the Portfolio UI: tracking add/remove is separate from allocation and snapshot testing controls.
- [x] Deterministic e2e mocks now have a live smoke counterpart, and the Playwright webServer harness now starts cleanly for both mocked and live smoke verification.
...
- [x] `npm.cmd run lint -- app/portfolio/page.tsx tests/e2e/portfolio-watchlist.spec.ts tests/e2e/portfolio-snapshot-history.spec.ts tests/e2e/helpers/portfolioPageMock.ts`
...
- [x] `npm.cmd run lint -- app/portfolio/page.tsx tests/e2e/portfolio-watchlist.spec.ts tests/e2e/portfolio-snapshot-history.spec.ts tests/e2e/market-overview.spec.ts tests/e2e/market-overview.live.spec.ts tests/e2e/corporate-comparison.spec.ts tests/e2e/helpers/*.ts`
- [x] `npm.cmd run lint -- playwright.config.ts`
- [x] `npm.cmd run lint -- app/portfolio/page.tsx app/corporate/page.tsx lib/benchmarkPresets.ts tests/e2e/portfolio-watchlist.spec.ts tests/e2e/portfolio-snapshot-history.spec.ts tests/e2e/corporate-comparison.spec.ts`
- [x] `npm.cmd run test:e2e -- tests/e2e/portfolio-snapshot-history.spec.ts`
- [x] `npm.cmd run test:e2e -- tests/e2e/portfolio-watchlist.spec.ts tests/e2e/portfolio-snapshot-history.spec.ts tests/e2e/corporate-comparison.spec.ts tests/e2e/market-overview.spec.ts`
