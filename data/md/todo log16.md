...
Docs and guideline follow-up:
- [x] Update `docs/architecture/visualization-metrics.md` so Monte Carlo chart semantics include guard-driven `loading`, `empty`, `invalid-data`, and `error` states instead of assuming a chart always renders when a result object exists
- [x] Update `docs/architecture/data-flow.md` so the Simulation Lab pipeline explicitly documents result normalization between worker output and React state, including dropped invalid rows, clamped numeric formatting, and warning propagation for partially recovered results
- [x] Update `guideline/verification-checks.txt` so the verification gate notes when targeted backend/API and Playwright coverage are required in addition to `tsc`, `lint`, and `build`
- [x] Keep future Monte Carlo doc updates tied to the shared ownership contract:
  - page-level state owns request lifecycle, normalization, warning visibility, and chart guard selection
  - worker code owns raw simulation math
  - charts consume validated view models rather than raw worker payloads
