...
- [x] Update architecture docs only if this changes the page-level cache ownership model rather than just tightening Corporate route behavior.
...
- [x] Verify focus trap, Escape close, backdrop close, and responsive modal sizing across all three modal types
- [x] Add internal loading and error-state coverage for each modal surface

### Phase 5 - Polish And Definition Of Done

Phase 5 note:
- Cross-tab UX audit has completed its first state-consistency pass. Market Overview and News now use explicit shared error states, Portfolio and Corporate use shared loading/empty/error surfaces in the remaining page-level drill-down gaps, and Monte Carlo was re-audited without requiring code changes.
- Progressive disclosure audit is now aligned with the spec: summary stays on page, deeper analysis stays in the page sections and tab workspaces, audit detail stays in the modal layer, and Portfolio stock drill-down now includes an explicit handoff to the canonical `/detail/[ticker]` route when route-level depth is needed.
- Explicit refresh behavior is now verified across the heavy-analysis tabs: Corporate and Portfolio remain refresh-gated on first load and after reload, Monte Carlo stays idle until a run button is used, and the valuation price helper does not fire until the user blurs the ticker field.

Cross-tab UX audit:
- [x] Apply consistent loading, empty, error, and stale-state patterns across Market Overview, Portfolio, Corporate, Monte Carlo, and News
- [x] Audit progressive disclosure levels: summary on page, analytics in expanded sections, audit detail in modals
- [x] Audit explicit refresh behavior so heavy analysis never auto-runs silently on mount

Design-system audit:
- [x] Remove ad hoc spacing values where token-based spacing should be used
- [x] Remove raw font-size usage where the type scale should be used
- [x] Confirm default cards stay flat by default and rely on border and spacing emphasis rather than shadows
- [x] Confirm Korean-market delta color semantics remain red-up and blue-down everywhere

Chart audit:
- [x] Standardize tooltip styling from shared config across all Recharts surfaces
- [x] Standardize axis label sizing and numeric formatting across all chart families
- [x] Verify chart empty and error states plus export affordances follow the chart-system spec

Responsive and accessibility audit:
- [x] Test desktop-primary behavior at `1280px`
- [x] Test tablet degradation at `768px`
- [x] Test mobile degradation at `375px`, especially for Market Overview and News
- [x] Verify Corporate and Monte Carlo remain usable, even if not fully optimized, on narrower widths
- [x] Check keyboard navigation, focus order, aria labels, and modal accessibility behavior

Verification gate before closing the design refactor track:
- [x] `npm.cmd run lint`
- [x] `npx tsc --noEmit`
- [x] `npm.cmd run build`
- [x] Run targeted page and component tests covering loading, empty, error, refresh-gated, and modal drill-down behavior
