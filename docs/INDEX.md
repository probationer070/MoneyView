...
| `metrics/README.md` | Metric reference entry point: what earns an entry, the eight-field template, seven family files, and the gaps the reference does not cover |
...
| `2026-09-18-watchlist-peer-sync-design.md` | Watchlist sync between the owner's PCs through a cloud-synced folder: one file per PC, per-ticker last-writer-wins with tombstones, and the public seed replaced by neutral defaults (draft) |
