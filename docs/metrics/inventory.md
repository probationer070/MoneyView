...
| `wacc` (`apps/api/models/schema_parts/decision.py:69`) | duplicate/snapshot of the WACC figure reported by `corporate_dcf.py` (see Follow-up candidates — the live figure itself is not one of this plan's 50 either). Populated directly as `round(float(metrics.wacc), 2)` at `apps/api/services/investment_decision.py:103` (and identically at `apps/api/services/corporate_comparison.py:355`) — same direct-from-`metrics` provenance as `roic` above, not `_dcf_snapshot`. **The surviving `wacc` entry (whichever family ultimately documents the live figure) must carry the same freezing note**: this is the WACC in force at decision time, and a later change to the live figure does not retroactively update it. |
...
| `relever_beta` (`packages/core_finance/beta.py:23`) | live since 2026-09-24: called by `_levered_beta_from_metrics` (`apps/api/services/corporate_comparison.py`), which feeds `capm_expected_return`. Before that its formula was hand-duplicated there. |
...
- **`packages/core_finance/risk_analysis.py` is exported but never reaches a reported number.** `payback_period`, `sensitivity_analysis`, and `monte_carlo_npv` have zero references anywhere under `apps/` (confirmed by repo-wide grep), yet all three are re-exported identically to `beta.py`'s functions at `packages/core_finance/__init__.py:25`. None of the spec's 50 candidates map to this file, and no family's description covers "risk analysis" as a topic — the spec's own family list corroborates that this module was never in scope, even though the task-1 brief names it as a file to check. Worth noting since it sharpens the contrast with `beta.py` below: `risk_analysis.py` has no test file at all (no `tests/core_finance/test_risk_analysis.py` or equivalent), where `beta.py` at least has `tests/core_finance/test_beta.py` exercising it. Neither module earning zero entries is a judgment about the code being deletable — it earns no entry because this is a reference of *reported* metrics, not an API surface audit.
...
- **`packages/core_finance/beta.py` is mostly unused.** `unlever_beta` and `bottom_up_beta` have zero callers under `apps/`. `relever_beta` was hand-duplicated at `apps/api/services/corporate_comparison.py` until 2026-09-24, when that site was changed to call it (the F2 fix).
...
- **The DCF report's base case and its own sensitivity grid use two independent implementations of the same formulas.** `apps/api/services/corporate_dcf.py:141-165` hand-computes projected FCFF, terminal value, and PV inline; the sensitivity grid built two lines later (`:230`) calls `packages/core_finance/dcf.py`'s `sensitivity_grid` → `multi_stage_dcf` → `calculate_terminal_value`/`calculate_npv` for the identical inputs. The `is_base` cell is documented (`dcf.py:236-239`) as reproducing the base valuation "exactly," but the two code paths are not the same code. **Correction (2026-09-09, during Task 7):** this bullet originally closed "it deserves a test, not an inventory row," which implied no such test existed. Two already did — `tests/api/test_corporate_dcf_sensitivity.py::test_the_base_cell_reproduces_the_reported_enterprise_value` (lines 92-102) asserts agreement only to `abs=0.01`, its comment naming the rounding-order reason, and the sibling `test_the_base_cell_reproduces_the_reported_per_share_value` (lines 105-113) carries that same step through the equity bridge at `abs=0.01/15.0`. Neither asserts exactness, so both corroborate the divergence rather than closing it. Task 6 then measured it live: maximum absolute per-share divergence $0.346 (TWIN), but a maximum *relative* divergence of **22.7% (MSTR**, whose base-cell and headline per-share figures are both near zero), with 1 of 109 comparable tickers above 1%. The full account — measurement, method, and what survives of the "small relative to either figure" framing — is in `docs/metrics/dcf-mechanics.md`'s `calculate_terminal_value` entry and is not duplicated here. What remains genuinely open is the grid's own cell semantics (`undefined_reason`, axis construction), which no entry covers; see `docs/metrics/README.md`'s "Known gaps".
