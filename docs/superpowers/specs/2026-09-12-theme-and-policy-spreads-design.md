...
> **Status:** designed and approved 2026-09-12, not yet implemented. Sub-project **I-D**,
...
All five spreads are the same thing — relative strength between two tickers over a window:

```
strength(A, B, t) = (A_t / A_0) / (B_t / B_0) × 100
```

Indexed to 100 at the window start, so a value above 100 means A outperformed B over the
window. `A_0` and `B_0` are the first closes at or after the window start.
...
**No new data class, and no new freshness policy.** These are daily price series, so they
join the existing daily-bars class already declared for stocks and indices: boundary daily
00:00 UTC, triggers schedule and watchlist add, yfinance with Stooq fallback, incremental
...
`^VIX` is an index and goes in `MARKET_INDICES`. The six ETFs are equities by `quoteType` and
take the stock OHLCV path.
...
{ "id": "ai", "label": "AI",
  ...
  "basis": "(BOTZ_t / BOTZ_0) / (GSPC_t / GSPC_0) x 100, indexed to 100 at window start",
  "series": [{ "date": "...", "value": 103.4 }],
  "latest": 103.4,
  "refused_reason": null }
```
